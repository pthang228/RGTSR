"""
RGTSR Training Pipeline - Two-Stage Training.

Stage 1: Learn temporal logical rules using TLogic's temporal random walks.
Stage 2: Train the rule-guided neural attention model (RGTSR).

Usage:
  python rgtsr_train.py --dataset ICEWS14 --data_format xerte --stage both
  python rgtsr_train.py --dataset ICEWS14 --stage 1  # Rule learning only
  python rgtsr_train.py --dataset ICEWS14 --stage 2 --rules_file rules.json
"""
...
# TLogic modules (reused directly)
...
# ===========================================================================
# Helper: Custom batch for DataLoader (from xERTE)
# ===========================================================================
...
def prepare_inputs(data, dataset='train', start_time=0):
    ...
    if dataset == 'train':
        events = data.train_idx
    elif dataset == 'valid':
        events = data.valid_idx
    elif dataset == 'test':
        events = data.test_idx
    else:
        raise ValueError("dataset must be 'train', 'valid', or 'test'")

    mask = events[:, 3] >= start_time
    return events[mask]


# ===========================================================================
# Stage 1: Rule Learning (from TLogic)
# ===========================================================================
def run_stage1_rule_learning(args, data):
    """
    Learn temporal logical rules using TLogic's temporal random walks.
    ...
    Returns:
        rules_dict: dict, relation_id -> list of rules
        rules_file: str, path to saved rules file
    """
    print("=" * 60)
    print("STAGE 1: Temporal Rule Learning (TLogic)")
    print("=" * 60)

    # Initialize temporal walk on training data
    temporal_walk = Temporal_Walk(
        data.train_idx, data.inv_relation_id, args.transition_distr
    )

    # Initialize rule learner
    rl = Rule_Learner(
        temporal_walk.edges, data.id2relation,
    ...
    for k, rel in enumerate(all_relations):
        for length in rule_lengths:
            ...
            for _ in range(args.num_walks):
                ...
                if walk_ok:
                    rl.create_rule(walk)
            it_time = round(time.time() - it_start, 4)
            n_rules = sum(len(v) for v in rl.rules_dict.values()) // 2
            print(f"  Relation {k+1}/{len(all_relations)}, length {length}: "
                  f"{it_time}s, total rules: {n_rules}")

    end = time.time()
    print(f"Rule learning finished in {round(end - start, 2)} seconds.")
    ...
    rules_file = f"{dt}_rgtsr_rules.json"
    rules_path = os.path.join(output_dir, rules_file)

    rules_save = {int(k): v for k, v in rl.rules_dict.items()}
    with open(rules_path, "w") as f:
        json.dump(rules_save, f)
...
def run_stage2_neural_training(args, data, rules_dict):
    """
    Train the RGTSR neural model with rule-guided sampling.

    Parameters:
        args: parsed arguments
        data: RGTSRData instance
        rules_dict: dict from Stage 1
    """
    print("=" * 60)
    ...
    if 'yago' in args.dataset.lower():
        ...
        time_granularity = 24
    elif 'wiki' in args.dataset.lower():
        time_granularity = 1
    else:
        time_granularity = 24
        print(f"Warning: unknown dataset, using time_granularity={time_granularity}")

    # Build Rule-Guided NeighborFinder
    nf = RuleGuidedNeighborFinder(
        adj, sampling=args.sampling, max_time=data.max_time,
    ...
    # Filter rules: keep only rules with sufficient confidence
    filtered_rules = {}
    for rel, rule_list in rules_dict.items():
        rel = int(rel)
        filtered = [
            r for r in rule_list
            if r['conf'] >= args.min_rule_conf
            and r.get('body_supp', 0) >= args.min_body_supp
        ]
        if filtered:
            filtered_rules[rel] = sorted(filtered, key=lambda x: x['conf'], reverse=True)
    print(f"Filtered rules: {sum(len(v) for v in filtered_rules.values())} rules "
          f"for {len(filtered_rules)} relations")

    # Build model
    model = RGTSR(
        nf, rules_dict=filtered_rules,
        num_entity=data.num_entities,
        num_rel=data.num_relations,
        emb_dim=args.emb_dim,
        DP_steps=args.DP_steps,
        DP_num_edges=args.DP_num_edges,
        alpha=args.alpha,
        ...
        ratio_update=args.ratio_update,
        device=device,
        diac_embed=args.diac_embed,
        emb_static_ratio=args.emb_static_ratio,
    ...
    for epoch in range(args.epoch):
        ...
        # Reload training data each epoch (for shuffling)
        train_loader = DataLoader(
            train_inputs, batch_size=args.batch_size,
    ...
    print(f"Best checkpoint: {os.path.join(checkpoint_dir, f'checkpoint_{best_epoch}.pt')}")
...
def validate(model, data, val_inputs, args, sp2o, val_spt2o, device):
    ...
    val_loader = DataLoader(
        val_inputs, batch_size=args.batch_size,
    ...
    with torch.no_grad():
        for sample in val_loader:
            ...
            target_rank_l, found_mask, target_rank_fil_l, target_rank_fil_t_l = \
                segment_rank_fil(
                    entity_att_score, entities, target_idx_l,
                    sp2o, val_spt2o, src_idx_l, rel_idx_l, cut_time_l
                )
    ...
    print(f"  Validation (filtered, time-dep): "
          f"H@1={hit_1_fil/num_query:.4f}, "
          f"H@3={hit_3_fil/num_query:.4f}, "
          f"H@10={hit_10_fil/num_query:.4f}, "
          f"MRR={mrr_fil/num_query:.4f}")
    print(f"  Validation (raw): "
          f"H@1={hit_1/num_query:.4f}, "
          f"H@3={hit_3/num_query:.4f}, "
          f"H@10={hit_10/num_query:.4f}, "
          f"MRR={mrr_total/num_query:.4f}, "
          f"Found={found_cnt/num_query:.4f}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="RGTSR Training Pipeline")
    ...
    parser.add_argument('--stage', type=str, default='both', choices=['1', '2', 'both'],
                        help='Which stage to run: 1=rules, 2=neural, both=both')
    parser.add_argument('--rules_file', type=str, default=None,
                        help='Path to pre-learned rules (for stage 2 only)')
    ...
    parser.add_argument('--transition_distr', type=str, default='exp', choices=['unif', 'exp'])
    parser.add_argument('--seed', type=int, default=12)

    # Stage 2: Neural training
    ...
    parser.add_argument('--sampling', type=int, default=5,
                        help='5=rule-guided (RGTSR default), 3=time-weighted (xERTE)')
    ...
    parser.add_argument('--alpha', type=float, default=0.5,
                        help='Weight for neural vs rule score: alpha*neural + (1-alpha)*rule')
    parser.add_argument('--rule_beta', type=float, default=2.0,
                        help='Boost factor for rule-matching edges during sampling')
    parser.add_argument('--min_rule_conf', type=float, default=0.01,
                        help='Minimum rule confidence for filtering')
    parser.add_argument('--min_body_supp', type=int, default=2,
                        help='Minimum body support for filtering')
    ...
    print("RGTSR - Rule-Guided Temporal Subgraph Reasoning")
    print(f"Dataset: {args.dataset}")
    print(f"Stage: {args.stage}")
    print(f"Config: alpha={args.alpha}, rule_beta={args.rule_beta}, "
          f"DP_steps={args.DP_steps}, sampling={args.sampling}")
    print()
    ...
    if args.stage in ['2', 'both']:
        if args.stage == '2':
            # Load pre-learned rules
            if args.rules_file is None:
                print("Warning: No rules file provided for stage 2. Running without rules.")
                rules_dict = {}
            else:
